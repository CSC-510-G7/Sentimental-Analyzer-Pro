...
# This method scraps article using google news that match the relevant query
# and dumps it on news.json file
def scrapNews(topicName, nums, jsonOutput=True):
    ...
    if cached_result:
        ...
        if jsonOutput:
            with open(
                "sentimental_analysis/realworld/news.json", "w"
            ) as json_file:
    ...
    if jsonOutput:
        f = "sentimental_analysis/realworld/news.json"
...
def getNewsResults(query, nums):
    ...
    base_url = (
        f"https://www.google.com/search?q={query}&gl=us&tbm=nws&num={nums}"
    )
